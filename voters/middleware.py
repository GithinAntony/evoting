...
class SessionMiddleware:
    ...
    def __call__(self, request):
        if request.path.startswith('/voters/'):
            ...
            for key, value in required_session_vars.items():
                ...
                # Check if the current URL is the login page
                    if not request.path.startswith(reverse('voters:sign_in')) and not request.path.startswith(reverse('voters:sign_up')) and not request.path.startswith(reverse('voters:signup_check_mobile')) and not request.path.startswith(reverse('voters:signup_check_email')):                
