...
class SessionMiddleware:
    ...
    def __call__(self, request):
        if request.path.startswith('/candidates/'):
            ...
            for key, value in required_session_vars.items():
                ...
                # Check if the current URL is the login page
                    if not request.path.startswith(reverse('candidates:sign_in')) and not request.path.startswith(reverse('candidates:sign_up')) and not request.path.startswith(reverse('candidates:signup_check_mobile')) and not request.path.startswith(reverse('candidates:signup_check_email')):                
